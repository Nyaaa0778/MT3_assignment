...
const char kWindowTitle[] = "LE2B_27_ヤマダ_ナオ_4_3_確認課題";
...
/// <param name="viewProjectionMatrix"></param>
/// <param name="viewportMatrix"></param>
void DrawPendulum(const Ball &ball, const ConicalPendulum &pendulum,
                  const Matrix4x4 &viewProjectionMatrix,
                  const Matrix4x4 &viewportMatrix) {
  // anchor → NDC → ビューポート
  Vector4 anchorClip = Transform4(pendulum.anchor, viewProjectionMatrix);
  Vector4 ballClip = Transform4(ball.pos, viewProjectionMatrix);

  if (anchorClip.w != 0.0f && ballClip.w != 0.0f) {
    Vector3 anchorNDC = ToNDC(anchorClip);
    Vector3 ballNDC = ToNDC(ballClip);

    Vector3 anchorScreen = Transform(anchorNDC, viewportMatrix);
    Vector3 ballScreen = Transform(ballNDC, viewportMatrix);

    // 糸を描画
    Novice::DrawLine(static_cast<int>(anchorScreen.x),
                     static_cast<int>(anchorScreen.y),
                     static_cast<int>(ballScreen.x),
                     static_cast<int>(ballScreen.y), 0xFFFFFFFF);

    // 支点も描画（小さな点 or 円）
    Novice::DrawEllipse(static_cast<int>(anchorScreen.x),
                        static_cast<int>(anchorScreen.y), 4, 4, 0.0f,
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
  ...
  Ball ball = {
      {conicalPendulum.anchor.x + radius, conicalPendulum.anchor.y - height,
       conicalPendulum.anchor.z}, // 位置
      8.0f                        // 半径
  };
  ...
  while (Novice::ProcessMessage() == 0) {
    ...
    if (isMoving) {
      conicalPendulum.angularVelocity =
          std::sqrtf(9.8f / (conicalPendulum.length *
    ...
    DrawPendulum(ball, conicalPendulum, worldViewProjectionMatrix,
                 viewportMatrix);
