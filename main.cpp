...
const char kWindowTitle[] = "LE2B_27_ヤマダ_ナオ_2_7_確認課題";
...
bool PointInsideAABB(const Vector3 &p, const AABB &aabb) {
  return (p.x >= aabb.min.x && p.x <= aabb.max.x) &&
         (p.y >= aabb.min.y && p.y <= aabb.max.y) &&
         (p.z >= aabb.min.z && p.z <= aabb.max.z);
}

bool CheckPlaneIntersection(int axis, float planeCoord, const Vector3 &start,
                            const Vector3 &dir, const AABB &aabb) {
  float originC, dirC;
  if (axis == 0) {
    originC = start.x;
    dirC = dir.x;
  ...
  // 方向ベクトルが0ならぶつからない or 並行してるだけ
  if (dirC == 0.0f) {
    return false; // この面とは交わらない
  }

  // tを計算
  float t = (planeCoord - originC) / dirC;

  // 線分範囲外(t < 0 or t > 1)なら、この線分上には交点がない
  if (t < 0.0f || t > 1.0f) {
    return false;
  }

  // 交点座標を計算
  Vector3 P = {start.x + dir.x * t, start.y + dir.y * t, start.z + dir.z * t};

  // 交点がこの面以外の2軸方向でAABBの範囲内にあるかチェック
  if (axis == 0) {
    // x = planeCoord だから、P.y, P.z を見る
    return (P.y >= aabb.min.y && P.y <= aabb.max.y) &&
           (P.z >= aabb.min.z && P.z <= aabb.max.z);
  } else if (axis == 1) {
    // y = planeCoord だから、P.x, P.z を見る
    return (P.x >= aabb.min.x && P.x <= aabb.max.x) &&
           (P.z >= aabb.min.z && P.z <= aabb.max.z);
  } else {
    // z = planeCoord だから、P.x, P.y を見る
    return (P.x >= aabb.min.x && P.x <= aabb.max.x) &&
           (P.y >= aabb.min.y && P.y <= aabb.max.y);
  }
}
...
bool IsCollision(const AABB aabb, const Segment &segment) {
  ...
  Vector3 end = {segment.origin.x + segment.diff.x,
                 segment.origin.y + segment.diff.y,
                 segment.origin.z + segment.diff.z};

  // 端が箱の中かチェック
  if (PointInsideAABB(start, aabb) || PointInsideAABB(end, aabb)) {
    return true; // 衝突あり
  }
  ...
  // 6つの平面を順番にチェック
  if (CheckPlaneIntersection(0, aabb.min.x, start, dir, aabb)) {
    return true;
  }
  
  if (CheckPlaneIntersection(0, aabb.max.x, start, dir, aabb)) {
    return true;
  }
  
  if (CheckPlaneIntersection(1, aabb.min.y, start, dir, aabb)) {
    return true;
  }

  if (CheckPlaneIntersection(1, aabb.max.y, start, dir, aabb)) {
    return true;
  }

  if (CheckPlaneIntersection(2, aabb.min.z, start, dir, aabb)) {
    return true;
  }

  if (CheckPlaneIntersection(2, aabb.max.z, start, dir, aabb)) {
    return true;
  }

  // 箱の中にも端点も外にも交点もないなら衝突なし
  return false;
}
...
/// <param name="screenVertices"></param>
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
  ...
  Segment segment{
      .origin{-0.7f, 0.3f, 0.0f}, .diff{2.0f, -0.5f, 0.0f}, .color{WHITE}};

  AABB aabb{.min{-0.5f, -0.5f, -0.5f}, .max{0.5f, 0.5f, 0.5f}, .color{WHITE}};
  ...
  while (Novice::ProcessMessage() == 0) {
    ...
    Novice::DrawLine(static_cast<int>(screenStart.x),
                     static_cast<int>(screenStart.y),
                     static_cast<int>(screenEnd.x),
                     static_cast<int>(screenEnd.y), segment.color);
